...
if (isset($_GET['addform']))
{
  ...
  if ($_POST['password'] != '')
  {
    $password = md5($_POST['password'] . 'fenthree');
...
if (isset($_POST['action']) and $_POST['action'] == 'Delete')
{
  ...
  // Get reviews belonging to author
  try
  {
    $sql = 'SELECT id FROM review WHERE authorid = :id';
    $s = $pdo->prepare($sql);
    $s->bindValue(':id', $_POST['id']);
    $s->execute();
  }
  ...
  {
    $error = 'Error getting list of reviews to delete.';
    include 'error.html.php';
    exit();
  }
  ...
  // Delete review category entries
  try
  {
    ...
    {
      $reviewId = $row['id'];
      $s->bindValue(':id', $reviewId);
      $s->execute();
    }
  }
  catch (PDOException $e)
  {
    $error = 'Error deleting category entries for review.';
    include 'error.html.php';
    exit();
  }

  // Delete reviews belonging to author
  try
  {
    $sql = 'DELETE FROM review WHERE authorid = :id';
    $s = $pdo->prepare($sql);
    $s->bindValue(':id', $_POST['id']);
    $s->execute();
  }
  ...
  {
    $error = 'Error deleting reviews for author.';
    include 'error.html.php';
    exit();
  }
