...
{
  $result = $pdo->query(
      'SELECT id, filename, mimetype, filedata, description
      FROM filestore');
}
catch (PDOException $e)
{
...
foreach ($result as $row)
{
  ...
  $imageType = $row['mimetype'];
  $base64Image = base64_encode($imageData);
  $thumbnail = 'data:' . $imageType . ';base64,' . $base64Image;
  $files[] = array(
      'id' => $row['id'],
      'filename' => $row['filename'],
      'mimetype' => $row['mimetype'],
      'description' => $row['description'],
      'thumbnail' => $thumbnail // This will be the source for displaying the image
